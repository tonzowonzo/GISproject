...
os.chdir("C:/Users/Tim/Desktop/BOKU/GIS/GISproject")
...
'''
columns = ["date", "day_of_year", "month", "year", "last_crop", "b", "g",
           "r", "nir", "swir1",  "swir2", "tirs1", "ndvi",
           "ndvi_ratio", "label", "binary_label"]
...
for field in field_areas:
    path = "C:/Users/Tim/Desktop/BOKU/GIS/GISproject/landsat_8/" + field
    for file in os.listdir(path + "/5/"):
        if file.endswith('.tif'):
            ...
            data = {"date": date, "day_of_year": day_of_year, "month": month,
                    "year": year, "last_crop": last_crop, "b": b, 
                    "g": g, "r": r, "nir": nir, "swir1": swir1,  
                    "swir2": swir2, "tirs1": tirs1, 
                    "ndvi": ndvi, "ndvi_ratio": ndvi_ratio, "label": label, 
...
# Loop for getting all files into the dataframe, labelled.
for field in field_areas:
    path = "C:/Users/Tim/Desktop/BOKU/GIS/GISproject/landsat_7/" + field
    for file in os.listdir(path + "/5/"):
        if file.endswith('.tif'):
            # The year, month and day time.
            date = file[:-4]
...
# Loop for getting all files into the dataframe, labelled.
for field in field_areas:
    path = "C:/Users/Tim/Desktop/BOKU/GIS/GISproject/landsat_5/" + field
    for file in os.listdir(path + "/5/"):
        if file.endswith('.tif'):
            # The year, month and day time.
            date = file[:-4]
            ...
            data = {"date": date, "day_of_year": day_of_year, "month": month,
                    ...
                    "g": g, "r": r, "nir": nir, "swir1": swir1,  
                    "swir2": swir2, "tirs1": tirs1, 
                    "ndvi": ndvi, "ndvi_ratio": ndvi_ratio,
                    "label": label, 
                    "binary_label": summer_or_winter_crop}
...
X_test_no_last_crop = X_test_no_last_crop.reset_index()
X_test_no_last_crop = X_test_no_last_crop.iloc[:, 1:]
# Define classifier.
...
from sklearn.preprocessing import LabelEncoder
# Get X and y training data
train_final = train_df[["day_of_year", "month", "year", "b", "g", "r", "nir", "swir1", 
                       "tirs1", "ndvi", "ndvi_ratio", "2a", "2b",
                          "3c", "4a", "label"]]
#train_final = train_final[train_final["year"] == 2011]
...
from sklearn.preprocessing import LabelEncoder
# Get X and y training data
test_final = test_df[["day_of_year", "month", "year", "b", "g", "r", "nir", "swir1", 
                       "tirs1", "ndvi", "ndvi_ratio", "2a", "2b",
                          "3c", "4a", "label"]]
# Encode train labels.
test_final["label"] = encoder.transform(test_final["label"])
#test_final = test_final[test_final["year"] == 2011]
...
###############################################################################
for_clf_2a = XGB_clf = XGBClassifier(learning_rate=0.02, n_estimators=200, silent=True,
                        objective="binary:logistic", scoring="roc_auc")
...
train_2a["label"] = train_2a["label"].where(train_2a["label"] == 0, 1)
...
test_2a["label"] = test_2a["label"].where(test_2a["label"] == 6, 0)
test_2a["label"] = test_2a["label"].where(test_2a["label"] == 0, 1)
...
###############################################################################
for_clf_3c = XGB_clf = XGBClassifier(learning_rate=0.02, n_estimators=200, silent=True,
                        objective="binary:logistic", scoring="roc_auc")
train_3c = train_final.copy()
# Create training data only with relevant crops.
train_3c = train_3c[train_3c["3c"] == True]
train_3c["label"] = train_3c["label"].where(train_3c["label"] == 1, 0)
train_3c["label"] = train_3c["label"].where(train_3c["label"] == 0, 1)
...
test_3c["label"] = test_3c["label"].where(test_3c["label"] == 0, 1)
...
train_4a["label"] = train_4a["label"].where(train_4a["label"] == 0, 1)
...
test_4a["label"] = test_4a["label"].where(test_4a["label"] == 0, 1)
